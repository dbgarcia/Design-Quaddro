  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'Não é permitido neste canal' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Canal não encontrado "${channel}"` })
      }
    }
    ...
    if (!email) {
      return res
      .status(400)
      .json({ msg: 'Informe um e-mail' })
    }

    if (!remail().test(email)) {
      return res
      .status(400)
      .json({ msg: 'E-mail invalido' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'É obrigatório concordar com o Cód/de/Conduta' })
    }

    invite({ token, org, email, channel: chanId }, err => {
      if (err) {
        if (err.message === `Redirecionando para o Slack...`) {
          return res
          .status(303)
      ...
      res
      .status(200)
      .json({ msg: 'PRONTO!. Check seu e-mail!' })
    })
  })
