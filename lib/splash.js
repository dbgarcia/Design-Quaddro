...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuários online do total de ',
          dom('b.total', total), ' registrados.'
        ]
        : [dom('b.total', total), ' os usuários são registrados até o momento.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('input.form-item type=email name=email placeholder=seuemail@email.com.br '
        + (!iframe ? 'autofocus' : '')),
      coc && dom('.coc',
        dom('label',
          dom('input type=checkbox name=coc value=1'),
          'Eu concordo com o ',
          dom('a', { href: coc, target: '_blank' }, 'Código de Conduta.'),
          '.'
        )
      ),
      dom('button.loading', 'ENVIAR MEU CONVITE!')
    ),
    dom('p.signin',
      'OU ',
